...
for epsilon in "${epsilons[@]}"; do
  for sample_size in "${sample_sizes[@]}"; do
    for particle in "${particles[@]}"; do
      ...
      sbatch <<EOT
#!/bin/bash
#SBATCH --job-name=rejection_simulation_eps${epsilon}_ss${sample_size}_p${particle}
#SBATCH --output=rejection_simulation_eps${epsilon}_ss${sample_size}_p${particle}_%j.log
#SBATCH --error=rejection_simulation_eps${epsilon}_ss${sample_size}_p${particle}_%j.err
#SBATCH --time=4:00:00
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
for i in {1..100}; do
  Rscript "$SCRIPT_DIR/rejection_abc_laplace.R" -d 1 -e $epsilon -n $sample_size -p $particle -i \$i  
done
