...
update_params_gibbs <- function(state,hyperparams=hyperparams){
  ...
  ###
  aa_star = hyperparams$d + state$N
  bb_star = 1 + state$S + state$N*(mu_new - state$Xbar)%*%t(mu_new - state$Xbar)
  
  #phi_new = rWishart(n=1,df=d_star,Sigma = W_star)[,,1]
  phi_new = rgamma(n=1, shape=aa_star/2, rate=bb_star/2)
...
run_chain = function(sdp, iteration=1,num_cycles=15000,N=1000,ep_N=NaN,eps,N_guess,
  ...
  # NOTE: For Comparing against our code, we fix the sdp
...
save(chain, file = paste("linear_regression/mcmc/conjugate_prior/data/mcmc_conj_n", N, "e", eps, "seed", seed, "i", iter, ".RData", sep = ""))
print("Saved Results")