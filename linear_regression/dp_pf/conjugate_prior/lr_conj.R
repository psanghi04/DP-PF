...
dprior <- function(thetaNew, hyper_params){ #OK
  logDst = dmvnorm(x = thetaNew$mu, mu = hyper_params$hp$theta, Sigma = matrix(hyper_params$hp$Sigma), log = T) +
    dchisq(x = thetaNew$Phi, df = hyper_params$hp$d, log = T) +
    dgamma(x = thetaNew$tau, shape = hyper_params$hp$a/2, scale = hyper_params$hp$b/2, log = T) +
    dmvnorm(x = thetaNew$beta[1:2], mu = hyper_params$hp$m, Sigma = solve(thetaNew$tau*hyper_params$hp$V), log = T)
  return(logDst)
}

rprior <- function(hyper_params, particles){ #OK
  muPrior = t(mvrnorm(n = hyper_params$N, mu = hyper_params$hp$theta, Sigma = hyper_params$hp$Sigma))
  PhiPrior = t(rchibarsq(n = hyper_params$N, df = hyper_params$hp$d))
  tauPrior = t(rgamma(n = hyper_params$N, shape = hyper_params$hp$a/2, scale = hyper_params$hp$b/2))
  betaPrior = sapply(1:hyper_params$N, function(k){
    mvrnorm(n = 1, mu = hyper_params$hp$m, Sigma = solve(tauPrior[k]*hyper_params$hp$V))
  })
...
rperturb <- function(t, mu, Phi, tau, beta, hyper_params, params_bound){ #OK
  ...
  #PhiNew = rtruncnorm(n = 1, a = params_bound$Phi[1], b = params_bound$Phi[2], mean = thetaOld[2,index], sd = variation) #df >= 2
  log_tauNew = mvrnorm(n = 1, mu = log(tau), Sigma = variation*diag(1,1))
  tauNew = exp(log_tauNew)
  #tauNew = rtruncnorm(n = 1, a = params_bound$tau[1], b = params_bound$tau[2], mean = thetaOld[3,index], sd = variation)
  betaNew = mvrnorm(n = 1, mu = beta[1:2], Sigma = variation*diag(1,2))
